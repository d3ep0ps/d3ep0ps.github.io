...
Він знаходиться у віртуальній мережевій фабриці — повністю розподіленій системі, яка оцінює правила задовго до того, як трафік досягне вашого ядра.
...
Ви побачите, як традиційні хостові фаєрволи мапляться — а іноді не мапляться — на хмарні концепції.
...
І якщо ви зрозумієте цю ментальну модель, ви будете траблшутити хмарні системи швидше, ніж більшість інженерів.
...
…фільтрують пакети у віртуальній мережевій фабриці, часто реалізованій всередині гіпервізора або розподіленого мережевого плану.
...
Це змінює те, як ви дебажите з'єднання:
...
* дебагінг може вплинути на глобальний трафік, а не тільки на один вузол
...
Коли ви дебажите хмарну мережу, ви повинні знати:
...
Як тільки ви зрозумієте шлях пакету, дебагінг хмарного фаєрволу стає детермінованим замість ворожіння.
...
**Хмарні фаєрволи повністю від'єднуються від ОС і прикріплюються до:**
...
* Azure NSG Flow Logs часто лагають і вимагають діагностичних воркспейсів.
...
Розуміння цієї різниці — це те, що робить траблшутинг хмарної мережі логічним, а не містичним.

---

## Поширені Помилки Хмарних Фаєрволів (і Чому Дебагінг Важчий, Ніж На Unix)

Коли інженери переходять з pf або iptables у хмарні середовища, вони часто припускають, що дебагінг з'єднання буде відчуватися так само.
Це не так.
...
На FreeBSD або Linux, якщо ваш фаєрвол дропає пакет:
...
Хмарні фаєрволи рідко логують рішення по правилах, а хостові фаєрволи ніколи не бачать дропнутого трафіку.
...
* NSG мають захардкоджену поведінку deny/allow
