...
До того, як я торкнувся Kubernetes, до того, як я написав Terraform або побудував multi-tenant SaaS архітектури, я провів більше десятиліття в окопах класичної інфраструктури.

Шість років у провайдері (ISP).
...
Кожна аварія, кожен тікет, кожна надзвичайна ситуація — ми вирішували це, розуміючи, як сервіси насправді працюють на системі.
...
* Якщо ви не розумієте Apache або Nginx, Kubernetes Ingress спантеличить вас.
* Якщо ви не розумієте Postfix, Amazon SES залишиться чорною скринькою.
* Якщо ви не розумієте DNS, налагодження *будь-якої* розподіленої системи стає ворожінням на кавові гущі.
...
**Урок:** Якщо ви не можете відлагодити сервіс, що працює нативно на хості, ви ніколи не налагодите його, коли він схований всередині контейнера.

### Фаза 2: Легковагова Ізоляція (Chroot & Jails)
Коли сервіси росли, нам потрібно було зупинити їхню боротьбу за ресурси хоста.
Unix представив **chroot** (зміну кореневої директорії), а пізніше **FreeBSD Jails** та **Solaris Zones**.

Це вирішило ранні проблеми multi-tenancy:
* Один зламаний PHP сайт не повинен читати `/etc/passwd`.
* Один падаючий поштовий сервер не повинен викликати паніку ядра.

Це було ефективно, але це не була повна ізоляція.
...
**Урок:** Віртуалізація навчила нас про **гарантії ресурсів** та **multi-tenancy**. Вона заклала фундамент для Хмари (AWS EC2 — це просто керована Віртуалізація).
...
Як тільки все запрацює на чистому залізі", ми перемістимося на один шар глибше: **chroot, FreeBSD Jails, Контейнери та Kubernetes.**
...
Вони навчили налагодження без дашбордів.
