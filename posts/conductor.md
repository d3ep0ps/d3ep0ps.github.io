...
In our last article, "Caging the Beast," we dismantled our monolithic server and isolated our services.
...
To scale, we need to stop treating our servers like "Pets" that we nurse to health. We need to treat them like "Cattle." We don't fix them; we replace them. (I hope this analogy isn't too harsh.)
...
> *"First, create a bridge0. Then, create a jail. Then, install Nginx."*
...
> *"There should be a bridge0. There should be a jail running Nginx."*
...
While this *works*, it introduces significant risks:
...
> P.S. A similar principle applies to virtual machines: Define configuration via Packer config -> build a Golden Image -> launch new virtual machines.
