...
У світі Unix існують дві основні філософії контролю: `pf` на стороні BSD та `iptables` (і його наступник, `nftables`) на Linux. Обидва захищають системи, обидва мають коріння, що сягає десятиліть, але вони походять з дуже різних шкіл думки.
...
Замість управління ланцюжками або таблицями криптичних правил, він читався як мова — майже англійською за структурою:
```
block in all
...
Жодних прихованих шарів, жодних демонів, що переписують ваші правила, жодного YAML чи XML на горизонті — просто чистий текстовий файл, `/etc/pf.conf`, який міг описати всю вашу мережеву позицію кількома рядками.
...
Це той вид зворотного зв'язку, який робить дебагінг і вивчення поведінки мережі інтуїтивним.
...
Навіть на скромному обладнанні `pf` може обробляти тисячі активних з'єднань без джиттера або втрати пакетів.
...
Ця складність ускладнює траблшутинг продуктивності, навіть коли "сира" пропускна здатність чудова.
...
І оскільки правила атомарні та читабельні, дрейф конфігурації (configuration drift) майже відсутній.
